...
struct AVLnode* search_recursive(struct AVLnode* node, Key key){
...
void fix_height(struct AVLnode* node){
...
struct AVLnode* insert_recursive(struct AVLnode* node, Item data){
	if (node == NULL){
...
struct AVLnode* delete_rightmost(struct AVLnode* node, struct AVLnode* deleted_node){
	if(node->right == NULL){
...
struct AVLnode* delete_leftmost(struct AVLnode* node, struct AVLnode* deleted_node){
	if(node->left == NULL){
		deleted_node->data = node->data;
		destruct_item((Item*)&(node->data));
		free(node);
...
struct AVLnode* delete_recursive(struct AVLnode* node, Key key){
	if(node == NULL){
		//not found
		return NULL;
	}
...
void visit_inorder_recursive(struct AVLnode* node,void (*visit)(Item)){
	if(node == NULL){
		return;
	}
...
void destruct_recursive(struct AVLnode* node){
	if(node == NULL)return;
