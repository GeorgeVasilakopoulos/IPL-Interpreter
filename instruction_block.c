...
struct instruction_block* connectBlocks(char*  ipli_file){
	loadFile(ipli_file);
	if(main_file==NULL){
...
void runBlockChain(struct instruction_block* first_block){
...
int main(void){
	...
	// mystr[0]='1';
	// mystr[1]='\0';
	// printf("%d\n",evaluateExpression(mystr));
	
	srand((unsigned int)time(NULL));

	struct instruction_block* first_block = connectBlocks("test.ipl");
	if(first_block == NULL)printf("Error in connecting blocks\n");
	else{
